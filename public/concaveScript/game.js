...
class Game {
        ...
        function tileClickHandler() {
            ...
            if (!player.getCurrentTurn() || !game) {
				...
				if(player.getPlayerColor()=='black' ){
					if(game.checkBlack33(row,col)==true){
					   alert('흑돌은 3x3 자리에 둘수없습니다.')
					   return;
					}
					if(game.checkBlack44(row,col)==true){
					   alert('흑돌은 4x4 자리에 둘수없습니다.')
					   return;
					}
					if(game.checkBlack66(row,col)==true){
	...
	checkBlack33(row,col){
	 ...
	 if(game.board[x][y] == 'b'){ // 금수는 흑에게만 적용
	     /*      3*3 판별 로직      */
	     // 가로 방향 카운팅
	     count_black = 1;
	     count_none = 0;
		 ...
		 if(x+i > 14)
		     break;
		 if(game.board[y][x+i] != 'w'){
		     if(game.board[y][x+i] != 'b'){
			 count_black++;
		     }else{
			 count_none++;
		     }
		 }else if(game.board[y][x+i] == 'w'){
		     count_white++;
		     break;
		 }
		 // 기본적으로 3칸까지만 카운팅 하고, 4번째 칸이 백돌일때만 추가 카운팅
		 if(i==3 && x+i+1 < 15)
		     if(game.board[y][x+i+1] == 'w')
			 count_white++;
	     }
	     // 가로 우 방향 열린 3 여부 체킹
		 ...
		 if(y+i > 14)
		     break;
		 if(game.board[y+i][x] != 'w'){
		     if(game.board[y+i][x] == 'b'){
			 count_black++;
		     }else {
			 count_none++;
		     }
		 }else if(game.board[y+i][x] == 'w'){
		     count_white++;
		     break;
		 }
		 // 기본적으로 3칸까지만 카운팅 하고, 4번째 칸이 백돌일때만 추가 카운팅
		 if(i==3 && y+i+1 < 15)
		     if(game.board[y+i+1][x] == 'w')
			 count_white++;
	     }
	     // 세로 하 방향 열린 3 여부 체킹
		 ...
		 if(y+i > 14 || x+i > 14)
		     break;
		 if(game.board[y+i][x+i] != 'w'){
		     if(_game.board[y+i][x+i] == 'b'){
			 count_black++;
		     }else {
			 count_none++;
		     }
		 }else if(game.board[y+i][x+i]== 'w'){
		     count_white++;
		     break;
		 }
		 // 기본적으로 3칸까지만 카운팅 하고, 4번째 칸이 백돌일때만 추가 카운팅
		 if(i==3 && y+i+1 < 15 && x+i+1 < 15)
		     if(game.board[y+i+1][x+i+1] == 'w')
			 count_white++;
	     }
	     // 대각선 우 방향 열린 3 여부 체킹
	...
	checkBlack44(row,col){
		return 
	}
	checkBlack66(row,col){
		return false;
